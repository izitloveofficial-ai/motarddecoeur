  ...
  head: () => ({
    meta: [
      { title: "Premium — Motards de Cœur" },
      {
        name: "description",
        content:
          "Aperçu des futures pistes premium Motards de Cœur, non disponibles au lancement de la pré-inscription.",
      },
      { property: "og:title", content: "Premium — Motards de Cœur" },
      {
        property: "og:description",
        content:
          "Découvrez la vision des futures offres premium Motards de Cœur avant l'ouverture commerciale.",
      },
  ...
  return (
    <Layout>
      {message && (
        <p
          role="status"
          className="mx-auto mt-8 max-w-3xl rounded-xl border border-primary/30 bg-primary/10 px-4 py-3 text-center text-sm"
        >
          {message}
        </p>
      )}
      <section className="px-4 py-12 sm:px-6 sm:py-20 text-center border-b border-border/40">
        <div className="mx-auto max-w-3xl animate-fade-up">
          ...
          <h1 className="font-display text-3xl sm:text-5xl md:text-7xl mb-6">
            Une expérience <span className="text-gradient-red italic">à construire</span>
          </h1>
          <p className="text-muted-foreground text-lg max-w-xl mx-auto">
            Les offres premium ne sont pas encore ouvertes. Aucun abonnement, paiement ou avantage
            commercial n'est actif pendant cette phase de pré-lancement.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 py-12 sm:px-6 sm:py-20">
            ...
            >
              ...
              <ul className="space-y-3 mb-10">
                {p.features.map((f, j) => (
                  <li
                    key={j}
                    className={`flex items-center gap-3 text-sm ${f.ok ? "text-foreground" : "text-muted-foreground/60 line-through"}`}
                  >
                    <span
                      className={`grid place-items-center w-5 h-5 rounded-full shrink-0 ${f.ok ? "bg-gradient-red" : "bg-muted"}`}
                    >
                      {f.ok ? (
        ...
        <div className="text-center mt-16 text-sm text-muted-foreground flex items-center justify-center gap-2">
          <Zap className="h-4 w-4 text-primary" /> Aucun paiement, abonnement ou espace premium
